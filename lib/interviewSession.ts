...
/** Atomic, so parallel turns on one grant cannot both write the same value. */
export async function consumeTurn(admin: SupabaseClient, grantId: string): Promise<void> {
  const { error } = await admin.rpc('consume_interview_turn', { p_grant_id: grantId })
  if (error) console.warn('Interview grant turn increment failed:', error.message)
}
