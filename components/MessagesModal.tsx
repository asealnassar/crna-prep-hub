...
const [compose, setCompose] = useState({
    ...
    selectedUserId: '',
    selectedUserIds: [] as string[],  // ADD THIS LINE ONLY
    selectedTier: 'free'
  })
...
<label className="block text-sm font-semibold text-gray-900 mb-3">Recipient</label>
                            ...
                            <input
                              ...
                              className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm mb-2"
                            />
                            ...
                            onChange={(e) => {
                              ...
                              setCompose({...compose, selectedUserIds: selected})
                            }}
className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 max-h-[150px] overflow-y-auto"
                          >
                              <option value="">Select a user...</option>
{allUsers
  .filter(u => 
    u.email.toLowerCase().includes(userSearchQuery.toLowerCase()) ||
    u.subscription_tier?.toLowerCase().includes(userSearchQuery.toLowerCase())
  )
  .map((u) => (
...
<p className="text-xs text-gray-500 mt-2">
                            {compose.selectedUserIds.length} user(s) selected
                          </p>
                          </div>
                        ) : (
                          <div>
